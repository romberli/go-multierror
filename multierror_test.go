...
func TestErrorError_custom(t *testing.T) {
	...
	multi := &Error{Errs: errors, ErrorFormat: fn}
	if multi.Error() != "foo" {
...
func TestErrorWrappedErrors(t *testing.T) {
	...
	multi := &Error{Errs: errors}
	if !reflect.DeepEqual(multi.Errs, multi.WrappedErrors()) {
...
func TestErrorUnwrap(t *testing.T) {
	t.Run("with errors", func(t *testing.T) {
		...
		for i := 0; i < len(err.Errs); i++ {
			...
			if !errors.Is(current, err.Errs[i]) {
